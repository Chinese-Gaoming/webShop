		...
		success : function(data) {//回调函数
			...
			for (var i = 0; i < result.length; i++) {
				...
				favourEle.append("<tr><td style='width:300px'>" + number
						+ "</td><td id='adminId"+i+"'  style='width:300px'>" + result[i].adminId
						+ "</td><td style='width:300px'>" + result[i].zcsj
						+ "</td><td style='width:300px'>" + result[i].remark
						+ "</td><td style='width:200px'><a onclick='deleteAdmin("+i
						+")'>删除<a></td></tr>");
			}
...
function deleteAdmin(i) {
  ...
  // alert(zzzz[i].adminId);
   var bb=zzzz[i].adminId;
	$.ajax({
		url : "deleteAdmin?AdminId="+bb+"",
		type : "get",
		dataType : "json",
		success : function(data) {//回调函数
			alert("删除成功");
